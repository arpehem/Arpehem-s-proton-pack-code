...
for(int cl=0;cl<64;cl++){
  red=max(red-10,0);
  cyclo_leds[cyclotron] = CRGB(red,0,0);
 controllers[1]->showLeds(gBrightness);
 controllers[0]->showLeds(blue);
 blue=max(blue-1,0);
 delay(10);
}
//delay(10);
...
if (powercell == 0){
  ...
  red=max(red-75,0);
  if(red>50){
    cyclo_leds[cyclotron_reset] = CRGB(red,0,0);
    if (bootnew==true){
     cyclo_leds[1] = CRGB(red,0,0);
     cyclo_leds[2] = CRGB(red,0,0); 
...
if (bootnew==true){
       cyclo_leds[1] = CRGB(0,0,0);
     cyclo_leds[2] = CRGB(0,0,0); 
  bootnew=false;
}

}
if (justvent==true){
  cyclo_leds[3] = CRGB(0,0,0); 
